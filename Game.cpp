...
void Game::Reset()
{
	...
	bricks.clear();
	int brickCount = 5;
	int space = WINDOW_WIDTH / brickCount;
	...
	for (int i = 0; i < brickCount; i++)
	{
		...
		brick.width = 10;
		brick.height = 2;
		brick.x_position = i * space + (space / 2 - brick.width / 2);
		brick.y_position = 5;
...
void Game::CheckCollision()
{
	...
	for (auto& brick : bricks)
	{
		if (brick.Contains(ball.x_position + ball.x_velocity, ball.y_position + ball.y_velocity))
		{
			...
			bricks.erase(std::remove_if(bricks.begin(), bricks.end(), [](const Box& brick) {return brick.color == ConsoleColor::Black;}), bricks.end());
		}
	}
